  ...
  <div id="app">
    <div class="container">
      <div class="row py-3">
          ...
          <table class="table table-hover mt-4">
            ...
            <tbody>
              <tr v-for="product in products" :key="product.id">
                ...
                <td width="120" class="text-danger">{{ product.price }}</td>
                <td width="150">
        ...
        <div class="col-md-6">
          <h2>單一產品細節</h2>
          <template v-if="nowChoseProduct.title">
            <div class="card mb-3">
              <img
                :src="nowChoseProduct.imageUrl"
                class="card-img-top primary-image"
                alt="主圖"
              />
              <div class="card-body">
                <h5 class="card-title">
                  {{ nowChoseProduct.title }}
                  <span class="badge bg-primary ms-2">{{
                    nowChoseProduct.category
                  }}</span>
                </h5>
                <p class="card-text">
                  商品描述：{{ nowChoseProduct.description }}
                </p>
                <p class="card-text">商品內容：{{ nowChoseProduct.content }}</p>
                <p class="card-text">
                  原價：{{ nowChoseProduct.origin_price }} 元 /
                  {{ nowChoseProduct.unit }}
                </p>
                <p class="card-text me-2">
                  售價：{{ nowChoseProduct.price }} 元 /
                  {{ nowChoseProduct.unit }}
                </p>
              </div>
            </div>
